...
- [iOS] Fix a `reloadAsync()` crash where the previous `AppContext` could deallocate before the old runtime finished tearing down, releasing cached JSI objects against a dying runtime on the wrong thread. The app context's lifetime is now tied to the runtime via native state attached to the `global.expo` object. ([#47098](https://github.com/expo/expo/pull/47098) by [@tsapeta](https://github.com/tsapeta))
